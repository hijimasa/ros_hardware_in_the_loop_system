...
- **PWMサーボ（コマンド方向）**: ロボット制御器が出力する PWM 信号を RP2040 の PIO で計測し、パルス幅と換算角度を ROS にパブリッシュする。シミュレータは「制御器が何を命令したか」を評価して仮想サーボの挙動を反映する。
- **ロータリエンコーダ（フィードバック方向）**: RP2040 の PIO により A/B 相のエンコーダパルスを高精度に生成し、制御器に供給する。
...
| RCサーボ（制御器出力の取り込み） | PWM 入力計測 | RP2040 PIO パルス幅計測 | ★★☆☆☆ | RP2040 (Pico H) | **実装済・動作確認済** |
