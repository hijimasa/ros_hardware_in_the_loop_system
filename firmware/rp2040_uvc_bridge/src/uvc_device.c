...
static const uint8_t blank_jpeg[] = {
    ...
    0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0x7B, 0x40, 0x01, 0x1F,
    0xFF, 0xD9,
};
...
void video_task(void) {
    ...
    if (rbuf->ready && rbuf->length > 0) {
        /* Copy frame to dedicated TX buffer so UART can freely reuse the frame buffer */
        uvc_tx_len = rbuf->length;
        memcpy(uvc_tx_buf, rbuf->data, uvc_tx_len);
        rbuf->ready = false;

        tx_busy = 1;
        tud_video_n_frame_xfer(0, 0, (void *)uvc_tx_buf, uvc_tx_len);
    } else {
        tx_busy = 1;
        tud_video_n_frame_xfer(0, 0, (void *)blank_jpeg, sizeof(blank_jpeg));
    }
}
